...
export function Fake3DShapePreview({ shape, label, className, color }: Fake3DShapePreviewProps) {
  ...
  return (
    <div className="inline-flex items-center gap-2">
      <div
        className={clsx(
          'relative flex items-center justify-center rounded-lg border border-white/10 backdrop-blur-md overflow-hidden',
          size
        )}
        style={{
          ...glowStyle,
          background: `linear-gradient(135deg, ${toRgba(baseColor, 0.18)}, ${toRgba(baseColor, 0.08)})`,
        }}
      >
        {shapes[shape]}
      </div>
