import { Copy, QrCode, ArrowRight, CheckCircle2, Shield, Info, Check } from 'lucide-react';
import { useState } from 'react';
import { WalletButton } from './WalletButton';
import { copyToClipboard } from '../../utils/clipboard';
import QRCode from 'react-qr-code';
import { WalletStepLayout } from './WalletStepLayout';

interface WalletReadyScreenProps {
  walletAddress: string;
  onContinue: () => void;
}

export function WalletReadyScreen({ walletAddress, onContinue }: WalletReadyScreenProps) {
  const [copied, setCopied] = useState(false);

  const handleCopy = async () => {
    const success = await copyToClipboard(walletAddress);
    if (success) {
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    }
  };

  return (
    <WalletStepLayout
      title="Wallet Created!"
      subtitle="Your secure Web3 wallet is ready"
      step={5}
      totalSteps={5}
      icon={(
        <div className="relative inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-[#00FFA3] to-[#06B6D4]">
          <div className="absolute inset-0 bg-gradient-to-br from-[#00FFA3] to-[#06B6D4] blur-2xl opacity-50 animate-pulse"></div>
          <CheckCircle2 className="w-9 h-9 sm:w-10 sm:h-10 text-[#0B0F1A] animate-bounce" style={{ animationDuration: '2s' }} />
        </div>
      )}
      background={(
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-1/4 left-1/2 w-80 h-80 sm:w-96 sm:h-96 bg-[#00FFA3] opacity-10 rounded-full blur-[120px]"></div>
          <div className="absolute bottom-1/4 right-1/3 w-80 h-80 sm:w-96 sm:h-96 bg-[#06B6D4] opacity-10 rounded-full blur-[120px]"></div>
        </div>
      )}
      actions={(
        <WalletButton
          onClick={onContinue}
          icon={ArrowRight}
        >
          Go to Dashboard
        </WalletButton>
      )}
    >
      <div className="space-y-3 sm:space-y-6">
        <div className="relative bg-white/5 backdrop-blur-lg border border-[#00FFA3]/20 rounded-xl p-4 sm:p-5">
          <div className="flex items-start gap-3 sm:gap-4">
            <div className="flex-shrink-0">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-[#00FFA3]/10 border border-[#00FFA3]/20 flex items-center justify-center">
                <Shield className="w-5 h-5 sm:w-6 sm:h-6 text-[#00FFA3]" />
              </div>
            </div>
            <div className="flex-1 space-y-1">
              <h3 className="text-white text-sm sm:text-base">Wallet Secured</h3>
              <p className="text-xs sm:text-sm text-gray-400 leading-relaxed">
                Your wallet is encrypted with AES-256-GCM and stored locally. Only you can access it with your password.
              </p>
            </div>
          </div>
        </div>

        <div className="space-y-2 sm:space-y-3">
          <h3 className="text-xs sm:text-sm text-gray-400 uppercase tracking-widest">Your Solana Address</h3>
          <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-4">
            <div className="flex items-center justify-between gap-2 sm:gap-3 mb-2">
              <p className="text-white text-sm sm:text-base break-all font-mono">{walletAddress}</p>
              <button
                onClick={handleCopy}
                className="flex-shrink-0 p-2 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-[#00FFA3]/50 rounded-lg transition-all"
              >
                {copied ? <Check className="w-4 h-4 text-[#00FFA3]" /> : <Copy className="w-4 h-4 text-gray-400" />}
              </button>
            </div>
            {copied && (
              <p className="text-xs text-[#00FFA3] animate-pulse">Copied to clipboard!</p>
            )}
          </div>

          <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-5 sm:p-6">
            <div className="flex flex-col items-center gap-3">
              <div className="bg-white p-3 sm:p-4 rounded-lg">
                <QRCode value={walletAddress} size={150} />
              </div>
              <p className="text-xs sm:text-sm text-gray-400 text-center">Scan to receive SOL</p>
            </div>
          </div>
        </div>

        <div className="space-y-2 sm:space-y-3">
          <h3 className="text-xs sm:text-sm text-gray-400 uppercase tracking-widest">How to Fund Your Wallet</h3>

          <div className="grid gap-2 sm:gap-3">
            <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-lg p-4">
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[#00FFA3]/20 border border-[#00FFA3]/30 flex items-center justify-center">
                  <span className="text-xs text-[#00FFA3]">1</span>
                </div>
                <div className="space-y-1">
                  <h4 className="text-white text-sm">Get SOL from an Exchange</h4>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Purchase SOL on exchanges like Coinbase, Binance, or Kraken
                  </p>
                </div>
              </div>
            </div>

            <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-lg p-4">
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[#00FFA3]/20 border border-[#00FFA3]/30 flex items-center justify-center">
                  <span className="text-xs text-[#00FFA3]">2</span>
                </div>
                <div className="space-y-1">
                  <h4 className="text-white text-sm">Withdraw to Your Address</h4>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Copy your address above and paste it as the withdrawal destination
                  </p>
                </div>
              </div>
            </div>

            <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-lg p-4">
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[#00FFA3]/20 border border-[#00FFA3]/30 flex items-center justify-center">
                  <span className="text-xs text-[#00FFA3]">3</span>
                </div>
                <div className="space-y-1">
                  <h4 className="text-white text-sm">Wait for Confirmation</h4>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Transactions typically take 1-3 minutes on Solana network
                  </p>
                </div>
              </div>
            </div>

            <div className="relative bg-white/5 backdrop-blur-lg border border-[#06B6D4]/20 rounded-lg p-4">
              <div className="flex items-start gap-3">
                <Info className="w-5 h-5 text-[#06B6D4] flex-shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <h4 className="text-white text-sm">Recommended Amount</h4>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    We recommend at least <span className="text-[#00FFA3]">0.06 SOL</span> for initial gameplay and transaction fees.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </WalletStepLayout>
  );
}